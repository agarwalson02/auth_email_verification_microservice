...
func (l *apiLogger) getLoggerLevel(cfg *config.Config) zapcore.Level {
	if level,ok:=loggerLevelMap[cfg.Logger.Level];ok{
		return level
	}
...
// Sync flushes any buffered log entries. Call this on app shutdown (e.g. defer appLogger.Sync()).
// Note: On Windows, syncing stderr is a no-op due to an OS limitation.
func (l *apiLogger) Sync() {
	_ = l.sugaredLogger.Sync() // ignore error: Windows returns "The handle is invalid" for stderr
}
